...
export const useInventoryDashboardData = () => {
  ...
  const loadData = useCallback(async () => {
    try {
      ...
      // Còn hàng (available > 0) lên đầu trang ; hết có sẵn / hết hàng xuống sau.
      normalizedRows.sort((a, b) => {
        const aHas = Number(a.totalAvailable || 0) > 0
        const bHas = Number(b.totalAvailable || 0) > 0
        if (aHas !== bHas) return aHas ? -1 : 1
        if (aHas && bHas) {
          const diff = Number(b.totalAvailable || 0) - Number(a.totalAvailable || 0)
