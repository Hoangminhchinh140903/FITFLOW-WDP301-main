...
const DASHBOARD_INTERVAL = { autoRefreshMs: 60000 }
const DASHBOARD_PAGINATION = { limit: 100, maxPages: 10 }

const KPI_STATUS_GROUPS = {
  successOrder: ['Completed', 'Returned'],
  finishedOrder: ['Completed', 'Returned', 'Cancelled', 'NoShow'],
  rentingFlow: ['Renting', 'WaitingReturn'],
  pickupQueue: ['PendingDeposit', 'Deposited', 'Confirmed', 'WaitingPickup'],
  inventoryReady: ['PendingDeposit', 'Deposited', 'Confirmed', 'WaitingPickup'],
  inventoryRenting: ['Renting'],
  inventoryLaundry: ['WaitingReturn'],
  inventoryRepair: ['Compensation'],
  issueOrder: ['Late', 'Compensation']
}
...
const DASHBOARD_ROUTES = {
  rentOrders: '/staff/orders/rent',
  returnOrders: '/staff/orders/return',
  saleOrders: '/staff/orders/sale',
...
const SUCCESS_ORDER_STATUSES = KPI_STATUS_GROUPS.successOrder
const FINISHED_ORDER_STATUSES = KPI_STATUS_GROUPS.finishedOrder
const RENTING_FLOW_STATUSES = KPI_STATUS_GROUPS.rentingFlow
...
const buildUrgentSoonItems = (orders, now) => {
  ...
  return orders
    .map((order) => {
      ...
      if (start && !Number.isNaN(start.getTime()) && KPI_STATUS_GROUPS.pickupQueue.includes(status)) {
        ...
        if (diff >= 0 && diff <= twoHoursMs) {
          return {
            ...
            severity: 'info',
            route: DASHBOARD_ROUTES.rentOrders,
          }
        }
      }
      ...
      if (end && !Number.isNaN(end.getTime()) && RENTING_FLOW_STATUSES.includes(status)) {
        ...
        if (diff >= 0 && diff <= twoHoursMs) {
          return {
            ...
            severity: 'warning',
            route: DASHBOARD_ROUTES.returnOrders,
          }
        }
      }
...
const buildUrgentLateItems = (orders, now) => {
  return orders
    .filter((order) => {
      const status = String(order?.status || '')
      if (status === KPI_STATUS_VALUES.late) return true
...
const buildUrgentIssueItems = (orders, bookings) => {
  const fromOrders = orders
    .filter((order) => KPI_STATUS_GROUPS.issueOrder.includes(String(order?.status || '')))
    .map((order) => ({
  ...
  const fromBookings = bookings
    .filter((booking) => String(booking?.status || '').toLowerCase() === KPI_STATUS_VALUES.bookingPending)
    .map((booking) => ({
...
const buildInventorySummary = (orders) => {
  ...
  const summary = orders.reduce((acc, order) => {
    ...
    if (KPI_STATUS_GROUPS.inventoryReady.includes(status)) acc.ready += itemCount
    if (KPI_STATUS_GROUPS.inventoryRenting.includes(status)) acc.renting += itemCount
    if (KPI_STATUS_GROUPS.inventoryLaundry.includes(status)) acc.laundry += itemCount
    if (KPI_STATUS_GROUPS.inventoryRepair.includes(status)) acc.repair += itemCount
    return acc
  }, initial)

  return [
    { key: 'ready', label: 'Sẵn sàng giao', value: summary.ready, hint: 'Đơn ở trạng thái chờ lấy', route: DASHBOARD_ROUTES.rentOrders, tone: 'success' },
    { key: 'renting', label: 'Đang thuê', value: summary.renting, hint: 'Đơn đang trong thời gian thuê', route: DASHBOARD_ROUTES.rentOrders, tone: 'info' },
    { key: 'laundry', label: 'Đang bảo dưỡng', value: summary.laundry, hint: 'Sản phẩm vừa trả cần xử lý', route: DASHBOARD_ROUTES.rentOrders, tone: 'warning' },
    { key: 'repair', label: 'Đang sửa', value: summary.repair, hint: 'Sản phẩm có phát sinh hư hỏng', route: DASHBOARD_ROUTES.rentOrders, tone: 'danger' },
  ]
}
...
const buildDailyPerformance = (ordersInRange, bookingsInRange, reviewStats) => {
  ...
  const lateCount = ordersInRange.filter((order) => String(order?.status || '') === KPI_STATUS_VALUES.late).length
  ...
  return {
    performanceName: 'Hiệu suất vận hành hôm nay',
...
export default function StaffDashboard() {
  ...
  const fetchAllRentOrders = useCallback(async () => {
    const limit = DASHBOARD_PAGINATION.limit
    let page = 1
    let pages = 1
    const rows = []
    ...
    do {
      ...
      page += 1
    } while (page <= pages && page <= DASHBOARD_PAGINATION.maxPages)
  ...
  const loadDashboard = useCallback(async () => {
    ...
    try {
      ...
      const quickStats = [
        { key: 'orders', label: 'Đơn hôm nay', value: ordersToday.length + saleOrdersToday.length, route: DASHBOARD_ROUTES.saleOrders, tone: 'primary', note: 'Tổng đơn thuê + đơn bán trong ngày' },
        { key: 'pickup', label: 'Chờ lấy đồ', value: orders.filter((order) => KPI_STATUS_GROUPS.pickupQueue.includes(String(order?.status || ''))).length, route: DASHBOARD_ROUTES.rentOrders, tone: 'info', note: 'Ưu tiên theo giờ hẹn' },
        { key: 'renting', label: 'Đang thuê', value: orders.filter((order) => String(order?.status || '') === KPI_STATUS_VALUES.renting).length, route: DASHBOARD_ROUTES.rentOrders, tone: 'success', note: 'Đang trong thời hạn thuê' },
        { key: 'return', label: 'Chờ trả', value: orders.filter((order) => String(order?.status || '') === KPI_STATUS_VALUES.waitingReturn).length, route: DASHBOARD_ROUTES.returnOrders, tone: 'warning', note: 'Cần kiểm đồ khi nhận' },
        { key: 'late', label: 'Trễ hạn', value: orders.filter((order) => String(order?.status || '') === KPI_STATUS_VALUES.late).length, route: DASHBOARD_ROUTES.rentOrders, tone: 'danger', note: 'Cần liên hệ khách ngay' },
      ]
      ...
      const dailyPerformance = buildDailyPerformance(ordersInRange, bookingsInRange, reviewStats)

      const payload = { quickStats, urgent, timeline, inventory, dailyPerformance }
  ...
  useEffect(() => {
    const timer = window.setInterval(() => {
      loadDashboard()
    }, DASHBOARD_INTERVAL.autoRefreshMs)
    ...
    <div className="space-y-6">
      <header className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm md:p-6">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            <p className="text-sm font-medium text-slate-500">FITFLOW STAFF DASHBOARD</p>
      ...
      {status === 'success' && data ? (
        <>
          ...
          <DashboardSection title="Hiệu suất làm việc" subtitle="Theo dõi tiến độ công việc trong ngày">
            <button
              type="button"
              onClick={() => navigate(data.dailyPerformance.route)}
            ...
            >
              <div className="flex flex-wrap items-start justify-between gap-3">
                <div>
                  <p className="text-base font-semibold text-slate-900">{data.dailyPerformance.performanceName}</p>
                  <p className="mt-1 text-sm text-slate-500">{data.dailyPerformance.owner}</p>
                </div>
                <span className="rounded-full bg-emerald-100 px-3 py-1 text-xs font-semibold text-emerald-700">
                  Đúng hạn {data.dailyPerformance.onTimeRate}%
                </span>
              </div>
              ...
              <div className="mt-4 grid gap-3 sm:grid-cols-3">
                <div className="rounded-xl bg-white p-3">
                  ...
                  <p className="mt-1 text-lg font-semibold text-slate-900">
                    {data.dailyPerformance.completedTasks}/{data.dailyPerformance.targetTasks}
                  </p>
                </div>
                <div className="rounded-xl bg-white p-3">
                  <p className="text-xs text-slate-500">Phản hồi booking TB</p>
                  <p className="mt-1 text-lg font-semibold text-slate-900">
                    {data.dailyPerformance.avgProcessMinutes > 0 ? `${data.dailyPerformance.avgProcessMinutes} phút` : 'Chưa đủ dữ liệu'}
                  </p>
                </div>
                <div className="rounded-xl bg-white p-3">
                  <p className="text-xs text-slate-500">Hài lòng khách hàng</p>
                  <p className="mt-1 text-lg font-semibold text-slate-900">
                    {data.dailyPerformance.customerSatisfaction > 0 ? `${data.dailyPerformance.customerSatisfaction}/5` : 'Chưa đủ dữ liệu'}
                  </p>
                </div>
              </div>
              ...
              <div className="mt-4">
                <div className="mb-1 flex justify-between text-xs text-slate-500">
                  ...
                  <span>{Math.round((data.dailyPerformance.completedTasks / data.dailyPerformance.targetTasks) * 100)}%</span>
                </div>
                <div className="h-2 rounded-full bg-slate-200">
                  <div
                    className="h-2 rounded-full bg-slate-900"
                    style={{ width: `${Math.min(Math.round((data.dailyPerformance.completedTasks / data.dailyPerformance.targetTasks) * 100), 100)}%` }}
                  />
                </div>
              </div>
            </button>
