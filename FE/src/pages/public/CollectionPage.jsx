...
export default function CollectionPage() {
  ...
  return (
    <div className="min-h-screen bg-[#f6f7ef] pb-20 text-[#10150f]">
      ...
      <main className="mx-auto w-full max-w-[1280px] space-y-6 px-4 py-5 md:px-6 lg:px-8">
        ...
        <section ref={productSectionRef} className="space-y-4">
          <div className="flex items-center justify-between gap-3">
            <div>
              <h2 className="text-[22px] font-black uppercase tracking-tight text-[#10150f]">Sản phẩm trong bộ sưu tập</h2>
              <p className="mt-1 text-xs font-semibold text-[#8d9788]">{filteredProducts.length} sản phẩm phù hợp</p>
            </div>
            <div className="flex items-center gap-2">
              <button
                type="button"
                onClick={() => setIsMobileFilterOpen(true)}
                className="inline-flex items-center gap-2 rounded-full border border-[rgba(16,21,15,0.15)] bg-[#fffaf0] px-4 py-2 text-xs font-black uppercase tracking-wider text-[#10150f] hover:bg-[#ebe7dc] transition duration-150 lg:hidden"
              >
      ...
      {isMobileFilterOpen && (
        <div className="fixed inset-0 z-50 bg-black/55 p-4 backdrop-blur-[2px] lg:hidden" onClick={() => setIsMobileFilterOpen(false)}>
          <div className="mx-auto max-w-md rounded-[28px] border border-[rgba(16,21,15,0.1)] bg-[#fffaf0] p-5 shadow-2xl" onClick={(event) => event.stopPropagation()}>
            <div className="mb-4 flex items-center justify-between">
              <h3 className="text-sm font-black uppercase tracking-wider text-[#10150f]">Bộ lọc collection</h3>
              <button
                type="button"
                onClick={() => setIsMobileFilterOpen(false)}
                className="inline-flex h-8 w-8 items-center justify-center rounded-full border border-[rgba(16,21,15,0.15)] bg-[#fffaf0] hover:bg-[#ebe7dc] transition"
              >
                <X size={16} />
              </button>
            </div>
      ...
      <div className="fixed bottom-4 left-1/2 z-40 w-[calc(100%-1.5rem)] -translate-x-1/2 rounded-[28px] border border-[rgba(16,21,15,0.1)] bg-[#fffaf0]/95 p-3 shadow-xl backdrop-blur md:hidden">
        <button
          type="button"
          onClick={() => productSectionRef.current?.scrollIntoView({ behavior: 'smooth', block: 'start' })}
          className="flex w-full items-center justify-between rounded-full bg-[#10150f] hover:bg-[#1b241b] px-6 py-3 text-xs font-black uppercase tracking-wider text-[#c9ff3d] transition duration-150 shadow-[0_10px_28px_rgba(201,255,61,0.38)]"
        >
          <span>Thuê ngay</span>
